...
#else
#include "board.h"                             // Debug Unit Cortex-M Processor Header File
#include "pio.h"
#endif

/// Processor Clock of the Cortex-M MCU used in the Debug Unit.
/// This value is used to calculate the SWD/JTAG clock speed.
#define CPU_CLOCK               (BOARD_MCK*1000000U)      ///< Specifies the CPU Clock in Hz.
...
__STATIC_INLINE void PORT_SWD_SETUP (void) {
	pio_set_output (PIN_RESET);
	pio_mode_select(PIN_RESET, PIO_OUTPUT_HIGH_SPEED);
	pio_set_output (PIN_SWCLK);
	pio_mode_select(PIN_SWCLK, PIO_OUTPUT_HIGH_SPEED);
	pio_set_output (PIN_SWDIO);
	pio_mode_select(PIN_SWDIO, PIO_OUTPUT_HIGH_SPEED);
}
...
__STATIC_INLINE void PORT_OFF (void) {
  pio_set_output (PIN_RESET);
  pio_mode_select(PIN_RESET, PIO_INPUT_PULLUP);
  pio_set_output (PIN_SWCLK);
  pio_mode_select(PIN_SWCLK, PIO_INPUT_PULLUP);
  pio_mode_select(PIN_SWDIO, PIO_INPUT_FLOATING);
}
...
__STATIC_FORCEINLINE uint32_t PIN_SWCLK_TCK_IN  (void) {
  return pio_get_state(PIN_SWCLK)>>PIN_SWCLK_POS;
}
...
__STATIC_FORCEINLINE uint32_t PIN_SWDIO_TMS_IN  (void) {
  return pio_get_state(PIN_SWDIO)>>PIN_SWDIO_POS;
}
...
__STATIC_FORCEINLINE void     PIN_SWDIO_OUT     (uint32_t bit) {
  if(bit) pio_set_output(PIN_SWDIO);
  else 	  pio_reset_output(PIN_SWDIO);
}
...
__STATIC_FORCEINLINE void     PIN_SWDIO_OUT_DISABLE (void) {
  pio_mode_select(PIN_SWDIO, PIO_INPUT_PULLUP);
}
...
__STATIC_FORCEINLINE uint32_t PIN_nRESET_IN  (void) {
  return pio_get_state(PIN_RESET)>>PIN_RESET_POS;
}
...
__STATIC_FORCEINLINE void     PIN_nRESET_OUT (uint32_t bit) {
  if(bit) pio_set_output(PIN_RESET);
  else 	  pio_reset_output(PIN_RESET);
}
...
__STATIC_INLINE void LED_RUNNING_OUT (uint32_t bit) {
  if(bit) pio_set_output(PIN_LED_RUN);
  else 	  pio_reset_output(PIN_LED_RUN);
}

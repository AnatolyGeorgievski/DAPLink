 ...
 */
#define CRC_SMALL_SIZE 1
static const uint16_t crc16_table[]=
{
	0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7,
	0x8108, 0x9129, 0xA14A, 0xB16B, 0xC18C, 0xD1AD, 0xE1CE, 0xF1EF,
#ifndef CRC_SMALL_SIZE
...
static const uint8_t CRC5_Lookup[]=
...
#endif
#ifndef CRC_SMALL_SIZE
uint8_t crc5_btm(uint8_t crc, unsigned char *ptr, unsigned int count)
{
	crc<<=3; // (0x1F<<3)
	if (count) do {
...
#ifndef CRC_SMALL_SIZE
uint8_t    crc8_smbus(uint8_t crc, unsigned char *ptr, size_t len) {
	while(len--){
...
static uint16_t	CRC16_update(uint16_t crc, unsigned char val){
	crc ^= val<<8;
//	crc = ((crc << 4) ^ (POLY16 * ((crc >> 12) & 0xF))) & CRC16_MASK;
//	crc = ((crc << 4) ^ (POLY16 * ((crc >> 12) & 0xF))) & CRC16_MASK;
...
int main()
{
	...
	uint8_t  c8 = crc8_smbus(0,test, 9);
	uint8_t  c5 = crc5_btm(0x1F, test, 9);
	...
	for (i=0; i<1; i++){
		...
		uint8_t  c8 = crc8_smbus(0, test, 9);
		test[9] = c8;
		uint8_t  c5 = crc8_smbus(0, test, 10);
